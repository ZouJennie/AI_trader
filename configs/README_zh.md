...
> 当前个人顾问版本只启用美股。A股配置和历史数据仅作存档，主程序会拒绝 `market: "cn"`。
...
### 代理类型 (agent_type)
当前使用 `BaseAgent` 进行美股研究。`BaseAgentAStock` 不再注册到运行入口。
...
- `execution_mode`: `advisory`只生成建议；`paper`允许写入模拟交易，默认使用`advisory`

### 每日运行时间 (schedule)
- `timezone`: 默认`America/New_York`，自动处理夏令时
- `analysis_time`: 默认`10:15`，即美股开盘45分钟后

### eToro费用
费用规则位于`configs/etoro_fees.json`。默认股票开仓、平仓各按1美元估算，实际使用前必须按照账户订单预览校准。

### SEC官方申报
在`.env`中设置`SEC_USER_AGENT="姓名 邮箱"`后，系统会检查10-Q、10-K、8-K等官方申报及其附件。SEC文本匹配只作为需要复核的风险证据，不能单独触发卖出。

### 趋势确认
本地行情工具计算MACD(12,26,9)、20/50/200日均线和20日成交量倍率。当天未完成的日K线不会进入指标；MACD只作确认，不是单独交易触发器。
