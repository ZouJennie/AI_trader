...
def get_latest_position(today_date: str, signature: str) -> Tuple[Dict[str, float], int]:
    ...
    # 如果当天有记录，返回当天的
    if max_id_today >= 0:
        return latest_positions_today, global_max_id
    ...
    # 返回上一个交易日的记录和全局最大ID
    if max_id_prev >= 0:
        return latest_positions_prev, global_max_id
    
    # 如果都没有记录，返回空
    return {}, global_max_id if global_max_id >= 0 else -1
