...
async def main(config_path=None):
    ...
    # This installation intentionally supports US equities only.
    market = config.get("market", "us")
    if market != "us":
        raise ValueError("Only the US market is supported by this configuration")
    ...
    execution_mode = agent_config.get("execution_mode", "advisory")
    if execution_mode not in {"advisory", "paper"}:
        raise ValueError("agent_config.execution_mode must be 'advisory' or 'paper'")
    ...
    for model_config in enabled_models:
        ...
        # Runtime state is isolated by signature for the agent process. MCP tools
        # that can mutate state receive signature/date explicitly.
        ...
        write_config_value("EXECUTION_MODE", execution_mode)
