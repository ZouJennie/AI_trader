...
from tools.indicator_tools import get_trend_snapshot


@mcp.tool()
def get_trend_analysis(symbol: str, as_of_date: str) -> Dict[str, Any]:
    """Calculate MACD, 20/50/200-day moving averages and volume signals.

    Only completed daily bars are used. If the current day's row has no close
    and volume, it is excluded automatically to prevent partial-bar signals.
    """
    try:
        _validate_date_daily(as_of_date)
        return get_trend_snapshot(symbol, as_of_date)
