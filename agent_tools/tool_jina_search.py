...
class WebScrapingJinaTool:
    ...
    def __call__(self, query: str, as_of_date: str) -> List[Dict[str, Any]]:
        print(f"Searching for {query}")
        all_urls = self._jina_search(query, as_of_date)
        return_content = []
    ...
    def _jina_search(self, query: str, as_of_date: str) -> List[str]:
        ...
        try:
            ...
            for item in json_data.get("data", []):
                ...
                # Undated results cannot prove point-in-time availability and
                # are therefore rejected for historical/advisory reproducibility.
                if standardized_date == "unknown" or standardized_date == raw_date:
                    continue

                # Compare normalized dates, allowing information published on
                # the analysis date while excluding future material.
                if as_of_date:
                    if standardized_date[:10] <= as_of_date[:10]:
...
@mcp.tool()
def get_information(query: str, as_of_date: str) -> str:
    """
    Use search tool to scrape and return main content information related to specified query in a structured way.

    Args:
        query: Key information or search terms to retrieve.
        as_of_date: Latest permissible publication date in YYYY-MM-DD format.
    ...
    try:
        ...
        datetime.strptime(as_of_date[:10], "%Y-%m-%d")
        results = tool(query, as_of_date)
