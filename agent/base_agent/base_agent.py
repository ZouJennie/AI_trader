...
from prompts.agent_prompt import STOP_SIGNAL, get_agent_system_prompt, get_investment_policy
...
class BaseAgent:
    ...
    def __init__(
        ...
        Args:
            ...
            market: Must be "us"
            execution_mode: "advisory" for recommendations or "paper" for simulated trades
        """
        self.signature = signature
        self.basemodel = basemodel
        if market != "us":
            raise ValueError("BaseAgent is configured for US stocks only")
        if execution_mode not in {"advisory", "paper"}:
            raise ValueError("execution_mode must be 'advisory' or 'paper'")
        self.market = "us"
    ...
    def _get_default_mcp_config(self) -> Dict[str, Dict[str, Any]]:
        ...
        services = {
            ...
            "sec": {
                "transport": "streamable_http",
                "url": f"http://localhost:{os.getenv('SEC_HTTP_PORT', '8005')}/mcp",
            },
            "trade": {
                "transport": "streamable_http",
    ...
    async def run_trading_session(self, today_date: str) -> None:
        ...
        self.agent = create_agent(
            self.model,
            tools=self.tools,
            system_prompt=get_agent_system_prompt(today_date, self.signature, self.execution_mode),
        )
        ...
        # create_agent already performs its internal tool loop. Calling it again
        # with copied tool output caused repeated and sometimes contradictory
        # decisions, so a daily session now has one authoritative invocation.
        try:
            response = await self._ainvoke_with_retry(message)
            agent_response = extract_conversation(response, "final") or ""
            agent_response, gate_blockers = enforce_entry_gate(agent_response, get_investment_policy())
            if gate_blockers:
                print(f"🛡️ BUY downgraded to HOLD: {'; '.join(gate_blockers)}")
            tool_msgs = extract_tool_messages(response)
            for tool_message in tool_msgs:
                content = getattr(tool_message, "content", str(tool_message))
                self._log_message(log_file, [{"role": "tool", "content": content}])
            self._log_message(log_file, [{"role": "assistant", "content": agent_response}])
            self._save_recommendation(today_date, agent_response)
    ...
    def _has_trade_record(self, today_date: str) -> bool:
        ...
        with open(self.position_file, "r", encoding="utf-8") as handle:
            for line in handle:
                ...
                if record.get("date") == today_date and action.get("action") in {"buy", "sell"}:
                    return True
        return False

    def _save_recommendation(self, today_date: str, content: str) -> None:
        recommendation_dir = os.path.join(self.data_path, "recommendations")
        os.makedirs(recommendation_dir, exist_ok=True)
        recommendation_file = os.path.join(recommendation_dir, "recommendations.jsonl")
        record = {
            "date": today_date,
            "signature": self.signature,
        ...
        with open(recommendation_file, "a", encoding="utf-8") as handle:
    ...
    def get_trading_dates(self, init_date: str, end_date: str) -> List[str]:
        ...
        if not os.path.exists(self.position_file):
            ...
            try:
                ...
                print(f"⚠️  Warning: Error reading position file {self.position_file}: {e}")
                # 如果读取文件出错，使用初始日期
                max_date = init_date

        # 确保 max_date 不为 None
        if max_date is None:
            print(f"⚠️  Warning: max_date is None, using init_date: {init_date}")
            max_date = init_date
        ...
        try:
            ...
            print(f"❌ Error parsing max_date {max_date}: {e}")
            # 如果 max_date 格式错误，使用初始日期
            max_date_obj = datetime.strptime(init_date, "%Y-%m-%d")
    ...
    async def run_date_range(self, init_date: str, end_date: str) -> None:
        ...
        if init_date is None or end_date is None:
            raise ValueError(f"日期参数不能为 None: init_date={init_date}, end_date={end_date}")
        ...
        print(f"📊 Trading days to process: {len(trading_dates)} days")
        if len(trading_dates) <= 10:  # 如果日期不多，显示全部
            print(f"   Dates: {trading_dates}")
        else:
            print(f"   First 5 dates: {trading_dates[:5]}")
